...
use crate::ty::{get_all_types, InpStruct, InpType};
    ...
    match typ {
        ...
        InpType::Pair(_, _) => {
            add_line_indented(
                input,
                indent,
                &format!("cin >> {0}.first >> {0}.second;", variable),
            );
        }
        InpType::Tuple(types) => {
        ...
        InpType::Vector(typ) => {
            ...
            add_line_indented(
                input,
                indent,
                &format!("for(int {0} = 0; {0} < {1}; {0}++){{", counter, length),
            );
...
pub fn struct_definition(
    line_tr: &str,
    structs: &mut String,
    defined_structs: &mut HashMap<char, InpStruct>,
) -> Result<(), ConfigError> {
