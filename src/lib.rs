...
pub fn cmd_watch(path: &Path, template_path: &Path, sub_args: &ArgMatches) -> Result<(), Box<dyn Error>> {
        ...
        for event in events {
            if event.mask.contains(EventMask::MODIFY) {
                if last_modified.elapsed() > Duration::from_secs(3) {
                    ...
                    println!("{} changed, will generate, compile and check.", path.to_string_lossy());
