...
public:
  ...
  struct Value { long long int x; };
  Value identity_value() { return { (long long int)1e9+1 }; }
  ...
  $class$(vector<Value> const& base)
    : n(next_power_of_two(base.size())),
      tree(2*n, identity_value()),
      lazy(2*n, identity_update()) {
    for (long long int i=0; i<(long long int)base.size(); ++i)
      tree[n+i] = base[i];
    build(1, 0, n);
  }

  // combines all values in range [l, r)
  Value query(long long int l, long long int r) {
    assert(0 <= l && l < r && r <= n);
    return query_(l, r, 1, 0, n);
  }

  long long int next_power_of_two(unsigned x) { return 1<<__lg(x-1)+1; }

  // updates all values in range [l, r)
  void update(long long int l, long long int r, Update a) {
    assert(0 <= l && l < r && r <= n);
...
private:
  ...
  void apply(long long int pos, Update a) {
  ...
  void build(long long int pos, long long int l, long long int r){
  ...
  Value query_(long long int ql, long long int qr, long long int pos, long long int l, long long int r) {
  ...
  void update_(Update a, long long int ql, long long int qr, long long int pos, long long int l, long long int r) {
